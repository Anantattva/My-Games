...
// this project aims to attemlpt how to optimize algorithm directly by filtering out unnecessary cases.

// result: very easy, only needed to edit one line, not so much rewarding.

// @date 22nd September, 2026
// @optimization
// I am adding more optimizations today
// Eliminated unnecessary sqrt checks
...
impl Position {
  ...
  pub fn get_dist_sq(&self, pos: &Position) -> f32 {
    let dx: f32 = pos.x - self.x;
    let dy: f32 = pos.y - self.y;
    ((dx * dx) + (dy * dy))
  }
...
impl Entity {
  ...
  pub fn apply_kinematics(&mut self) {
    ...
    if self.vel.get_speed_sq() <= 0.00001 { // removed sqrt here
      // << quit motion >> \\
      self.vel.vx = 0.0;
      self.vel.vy = 0.0;
      // << turn off flag >> \\
  ...
  pub fn check_pocketed(&mut self) {
    ...
    for i in 0..4 {
      ...
      let dist: f32 = self.pos.get_dist_sq(&corner);  // removed sqrt here
      let threshold: f32 = 5.0 * U * U;
      if dist <= threshold {
  ...
  pub fn get_closest_pocket(&self) -> Position {
    ...
    let mut min_dist: f32 = self.pos.get_dist_sq(&pocket);  // removed sqrt here
    for i in 1..4 {
      let target: Position = BOARD_CORNERS[i as usize];
      let dist: f32 = self.pos.get_dist_sq(&target);        // removed sqrt here
      if dist < min_dist {
        pocket = target;
        min_dist = dist;
      }
  ...
  pub fn apply_uniform_momentum_resolve(&mut self) {
    for i in 0..(MAX_ENTITIES - 1) {
      ...
      right.iter_mut().for_each(|target| {
         ...
         */
        let threshold: f32 = 11.0 * U * U;
        if entity.pos.get_dist_sq(&target.pos) <= threshold { // removed sqrt here
...
fn estimate_best_shot(current_pos: &AllEntities) {
  ...
  */
  ...
  println!("Time for one AI simulation across 4 threads: {}ms.", time);
...
fn main() {
    ...
    estimate_best_shot(&mut board); // better: 80ms on average; best observed: 53ms
}
