...
export function preprocessShader() {
  ...
  shader = shader.replace("{{LOGICAL_WIDTH}}", logicalWidth.toFixed(4));
  shader = shader.replace("{{LOGICAL_HEIGHT}}", logicalHeight.toFixed(4));
  ...
  // << inject board unit >> \\
  shader = shader.replace("{{BOARD_UNIT}}", u.toFixed(4));
...
export async function estimateBestShotGPU() {
  ...
  try {
    ...
    console.log(`Total time for GPU AI simulation: ${time.toFixed(3)}ms.`);
    console.log(`Time for GPU simulation: ${(stamp2 - stamp1).toFixed(3)}ms.`);
