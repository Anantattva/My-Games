...
fn resolve_momentum(entities: ptr<function, array<Entity, MAX_ENTITIES>>, i: u32, j: u32) {
  ...
  if ((*entities)[i].is_gliding == 0.0 && (*entities)[j].is_gliding == 0.0) {
    return;
  }
...
fn estimate_best_shot(@builtin(global_invocation_id) id: vec3u) {
  let index: u32 = id.x;
  ...
  // << local thread copy >>
  var clone_entities: array<Entity, MAX_ENTITIES> = initial_entities;
  let traj: TrajectoryParams = trajectories[index];

  // << apply trajectory to striker >>
  if (clone_entities[0].type_id == 0.0 && clone_entities[0].is_active == 1.0) {
    clone_entities[0].pos.x = traj.x;
    clone_entities[0].pos.y = traj.y;
    clone_entities[0].vel.x = traj.p * cos(traj.a);
    clone_entities[0].vel.y = traj.p * sin(traj.a);
    clone_entities[0].pos += clone_entities[0].vel;
    clone_entities[0].is_gliding = 1.0;
  ...
  // << calculate score directly using pointer >>
  score_results[index] = simulate_score(&clone_entities);
}
`;