...
// !! IMPORT MODULES !! \\
import { dpr, vw, vh } from "./Constants.js";
...
import * as Parallel_CPU_AI from './Parallel_AI.js';

////// +++ BOARD +++ \\\\\\\
/** @type {HTMLCanvasElement} */
const board = document.getElementById('board');
board.width = logicalWidth * dpr;
board.height = logicalHeight * dpr;
...
class Orchestrator {
  ...
  /**
   * Calls multi-threaded CPU AI.
   * 
   * @function call_Parallel_CPU_AI
   * @param {Number} [threads = navigator.hardwareConcurrency]
   * @return {void}
   */
  call_Parallel_CPU_AI(threads = navigator.hardwareConcurrency) {
    Parallel_CPU_AI.estimateBestShotParallel(threads).then((shot) => {
      const striker = PureBeing.striker;
      if (shot) {          
        striker.x = shot.x;
        Engine.launchStriker(shot.a, shot.p);
        this.inputs.wasShot = true;
        this.IS_AI_THINKING = false;
        console.log("Multi-threaded CPU AI Shot!");
      } else {
        // << gracefully fallback to CPU AI >> \\
        this.call_CPU_AI();
      }
 ...
 * Engine calls async function estimateBestShot that yield thread controls back-to-back per 60 simulation
 * Finally, when all simulations are done, AI yields & launches carrom at best estimated shot.
 * 
 * GPU AI WGSL shader is in file __Engine_Shader.js__.
 * WebGPU is in __GPU_AI.js__.
 * 
 * On 22nd September, 2026, I implemented a multi-threaded JS version using Web Workers.
 * For which I had to introduce __Constants.js__ to store constants safely & separately.
 * The AI file is __Parallel_AI.js__.
 * Worker file is __AI_Worker.js__.
 * 
 * +++++++ GAME RULES +++++++
 ...
 *   call_CPU_AI - plays the CPU AI opponent.
 *   call_GPU_AI - plays the GPU AI opponent.
 * 
 * @flags
